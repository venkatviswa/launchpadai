...
- **Generated projects must be runnable out of the box** — agent with one example tool, uniform entrypoint, passing imports. Never empty skeletons, and never `NotImplementedError` stubs behind a wizard option: if an option is offered, its generated code must work.
...
- **Testing:** pytest. Every adapter has structural generation tests (unit + validation) and a render → install → import smoke test (marked `slow`, run per-adapter in CI). Run tests before declaring done.
